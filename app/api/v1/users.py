...
async def create_user(user: UserCreate, db: Session = Depends(get_db_dependency)):
    return await UserService.create_user(db, user)


@router.get('/{user_id}', status_code=status.HTTP_200_OK, response_model=UserOut)
async def get_user(user_id: UUID4, db: Session = Depends(get_db_dependency)):
    return await UserService.get_user(db, user_id)


@router.put('/{user_id}', status_code=status.HTTP_200_OK, response_model=UserOut)
async def update_user(user_id: UUID4, user: UserUpdate, db: Session = Depends(get_db_dependency)):
    return await UserService.update_user(db, user_id, user)


@router.delete('/{user_id}', status_code=status.HTTP_204_NO_CONTENT)
async def delete_user(user_id: UUID4, db: Session = Depends(get_db_dependency)):
